...
ngOnInit() {
  ...
  const draftId = Number(this.route.snapshot.queryParams['draftId']);
  ...
  if (this.planId) {
    this.planService.getPlanById(this.planId).subscribe(plan => {
      ...
      // ✅ If draftId exists, load draft data after plan loads
      if (draftId) {
...
saveOrUpdateDraft() {
  ...
  // 1. Map Premium Frequency string to Numeric Enum if your backend requires it
  // Monthly = 0, Quarterly = 1, Yearly = 2 (Check your Domain.Enums.PremiumFrequency)
...
handleError(err: any) {
  ...
  if (err.status === 400 && err.error.errors) {
    ...
    // This will print exactly which field (.e.g "PremiumFrequency") failed
  }
}
...
submit() {
  this.processing = true; // Use your existing processing flag
